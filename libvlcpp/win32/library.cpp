...
Library * Library::hLibrary = 0;

Library::Library() : hModule(0)
{
        HKEY hKey;
        ...
        if(RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\VideoLan\\VLC"), 0, regSamDesired, &hKey) != ERROR_SUCCESS) {
                ...
                throw VLC::Exception("Failed Windows API call (RegQueryValue)", GetLastError());
        }
        ...
        if(!SetDllDirectory(TEXT("VLC"))) {
                throw VLC::Exception("Failed Windows API call (SetDllDirectory)", GetLastError());
        }
        ...
        if(!(hModule = LoadLibrary(TEXT("libvlc.dll")))) {
                throw VLC::Exception("Failed load VLC (LoadLibrary)", GetLastError());
        }
        ...
        if(!(m_libvlc_errmsg = (proc_libvlc_errmsg)GetProcAddress(hModule, "libvlc_errmsg"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_errmsg)", GetLastError());
        }
        if(!(m_libvlc_clearerr = (proc_libvlc_clearerr)GetProcAddress(hModule, "libvlc_clearerr"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_clearerr)", GetLastError());
        }

        if(!(m_libvlc_get_version = (proc_libvlc_get_version)GetProcAddress(hModule, "libvlc_get_version"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_get_version)", GetLastError());
        }

        if(!(m_libvlc_new = (proc_libvlc_new)GetProcAddress(hModule, "libvlc_new"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_new)", GetLastError());
        }
        if(!(m_libvlc_vlm_release = (proc_libvlc_vlm_release)GetProcAddress(hModule, "libvlc_vlm_release"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_vlm_release)", GetLastError());
        }

        if(!(m_libvlc_vlm_add_broadcast = (proc_libvlc_vlm_add_broadcast)GetProcAddress(hModule, "libvlc_vlm_add_broadcast"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_vlm_add_broadcast)", GetLastError());
        }
        if(!(m_libvlc_vlm_add_vod = (proc_libvlc_vlm_add_vod)GetProcAddress(hModule, "libvlc_vlm_add_vod"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_vlm_add_vod)", GetLastError());
        }
        if(!(m_libvlc_vlm_change_media = (proc_libvlc_vlm_change_media)GetProcAddress(hModule, "libvlc_vlm_change_media"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_vlm_change_media)", GetLastError());
        }
        if(!(m_libvlc_vlm_play_media = (proc_libvlc_vlm_play_media)GetProcAddress(hModule, "libvlc_vlm_play_media"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_vlm_play_media)", GetLastError());
        }
        if(!(m_libvlc_vlm_pause_media = (proc_libvlc_vlm_pause_media)GetProcAddress(hModule, "libvlc_vlm_pause_media"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_vlm_pause_media)", GetLastError());
        }
        if(!(m_libvlc_vlm_seek_media = (proc_libvlc_vlm_seek_media)GetProcAddress(hModule, "libvlc_vlm_seek_media"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_vlm_seek_media)", GetLastError());
        }
        if(!(m_libvlc_vlm_stop_media = (proc_libvlc_vlm_stop_media)GetProcAddress(hModule, "libvlc_vlm_stop_media"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_vlm_stop_media)", GetLastError());
        }
        if(!(m_libvlc_vlm_del_media = (proc_libvlc_vlm_del_media)GetProcAddress(hModule, "libvlc_vlm_del_media"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_vlm_del_media)", GetLastError());
        }
        if(!(m_libvlc_vlm_show_media = (proc_libvlc_vlm_show_media)GetProcAddress(hModule, "libvlc_vlm_show_media"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_vlm_show_media)", GetLastError());
        }

        if(!(m_libvlc_vlm_set_enabled = (proc_libvlc_vlm_set_enabled)GetProcAddress(hModule, "libvlc_vlm_set_enabled"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_vlm_set_enabled)", GetLastError());
        }
        if(!(m_libvlc_vlm_set_loop = (proc_libvlc_vlm_set_loop)GetProcAddress(hModule, "libvlc_vlm_set_loop"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_vlm_set_loop)", GetLastError());
        }
        if(!(m_libvlc_vlm_set_mux = (proc_libvlc_vlm_set_mux)GetProcAddress(hModule, "libvlc_vlm_set_mux"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_vlm_set_mux)", GetLastError());
        }
        if(!(m_libvlc_vlm_set_output = (proc_libvlc_vlm_set_output)GetProcAddress(hModule, "libvlc_vlm_set_output"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_vlm_set_output)", GetLastError());
        }
        if(!(m_libvlc_vlm_set_input = (proc_libvlc_vlm_set_input)GetProcAddress(hModule, "libvlc_vlm_set_input"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_vlm_set_input)", GetLastError());
        }
        if(!(m_libvlc_vlm_add_input = (proc_libvlc_vlm_add_input)GetProcAddress(hModule, "libvlc_vlm_add_input"))) {
                throw VLC::Exception("Failed get process address (GetProcAddress: -> libvlc_vlm_add_input)", GetLastError());
        }
        ...
        if(GetModuleFileName(hModule, lpInstallDir, sizeof(lpInstallDir)) == 0) {
                throw VLC::Exception("Failed Windows API call (GetModuleFileName)", GetLastError());
        }
}
...
Library::~Library() 
{
        if(hModule) {
                if(!FreeLibrary(hModule)) {
                        throw VLC::Exception("Failed close VLC (FreeLibrary)", GetLastError());
                }
        }
}
